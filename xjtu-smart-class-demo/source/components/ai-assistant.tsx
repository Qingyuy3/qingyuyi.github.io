...
export function AIAssistant({ compact = false, onOpenMaterial }: AIAssistantProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-2xl border-t-[6px] border-t-brand-red bg-brand-blue text-white shadow-[0_16px_40px_rgba(35,68,119,0.18)] ${compact ? 'p-5 sm:p-7' : 'p-6 sm:p-8'}`}>
      <div className="relative">
        ...
        <form onSubmit={(event) => { event.preventDefault(); runSearch(query); }} className="mt-5 flex gap-2 rounded-2xl bg-white p-2 shadow-lg">
          ...
          <Button type="submit" size="icon-lg" className="rounded-xl bg-brand-red hover:bg-[#b91f16]" aria-label="开始检索"><ArrowRight /></Button>
        </form>
        ...
        {submittedQuery && (
          <div className="mt-5 rounded-2xl bg-white p-4 text-slate-900 shadow-xl sm:p-5">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-sm font-medium"><Search className="size-4 text-brand-red" />“{submittedQuery}”的检索结果</div>
